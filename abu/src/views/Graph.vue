<template>
  <div>
    ...
    <v-textarea v-model="gnuplotText"> </v-textarea>
    <v-btn @click="writeGs">書き込み</v-btn>
  </div>
</template>
<script lang="ts">
import Vue from "vue";
...
export default Vue.extend({
  ...
  computed: {
    gnuplotText() {
      ...
      text += `set autoscale\n`;
      text += ` plot "-" u($1*0.1):($4):($5)  w filledcu lc rgb "${this.fcolorHex}" notitle, "-" u ($1*0.1):($2) w l lc rgb "${this.lcolorHex}" notitle; `;
      return text;
    },
    fcolorHex: {
      set(newval: string) {
  ...
  methods: {
    writeGs() {
      const command = `echo '${this.gnuplotText}' > /home/endo/Documents/2papp/process_bin/newcppera/evescan/graph/sin.gs`;
